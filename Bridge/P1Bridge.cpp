...
JNIEXPORT jdouble JNICALL Java_P1Bridge_compute(JNIEnv *env, jobject thisObj, jint numthreads, jint modo, jint longArray, jint op) {

    //Orden de los argumentos ( nombrefichero, longArray , Operacion, mumThreads )
    // ( mbrefichero, longArray , Operacion, --multithread, numthreads )
    ...
    if(bothresult[0] == bothresult[1]){
        ...
        std::cout <<"Tiempo de hilos computando = "<< (Tfinal - Tinicial) <<" microsegundos\n";
...
void ejecucionThread (int id ,struct data_information* data, int inicio, operation op, int longArray){
    ...
    if(inicio != (longArray)){ //si no estoy al final del array, calculo
        ...
        if(op == SUM){
            ...
            std::cout <<"Soy el thread "<< id <<", Uso Atomicos Resultado global = "<< data->resultado <<"\n";
        #endif
